...
function prepareTaskData(taskTemplate, state, index, adminUsers, operatorUsers) {
  ...
  const useAdminAsCreator = Math.random() < 0.3;
  const creatorIndex = index % (useAdminAsCreator ? adminUsers.length : operatorUsers.length);
  const creator = useAdminAsCreator ? adminUsers[creatorIndex] : operatorUsers[creatorIndex];
  
  let revisorIndex = index % adminUsers.length;
  if (useAdminAsCreator && revisorIndex === creatorIndex) {
    revisorIndex = (revisorIndex + 1) % adminUsers.length;
  }
  const revisor = adminUsers[revisorIndex];
